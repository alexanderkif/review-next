...
const PrintControls = ({ className = '' }: PrintControlsProps) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      const darkColor = rgb(0.12, 0.16, 0.23); // #1e293b
      ...
      const lineHeight = 12;
      let pageAnnotations: any[] = []; // Collect annotations for current page
      ...
      const wrapTextWithBullets = (text: string, maxWidth: number, fontSize: number, font: any) => {
      ...
      const drawTextWithGreenBullets = (text: string, x: number, yPos: number, fontSize: number, font: any, defaultColor: any) => {
  ...
  return (
    <div className={`flex gap-3 ${className}`}>
      <a
        ...
        className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium bg-gradient-to-r from-emerald-600 to-blue-600 text-white shadow-[3px_3px_6px_#9ca3af,_-1.5px_-1.5px_6px_rgba(255,255,255,0.1),_inset_-2px_-2px_1px_rgba(0,0,0,0.3)] hover:shadow-[4.5px_4.5px_9px_#9ca3af,_-2.25px_-2.25px_9px_rgba(255,255,255,0.1),_inset_-2px_-2px_1px_rgba(0,0,0,0.3)] hover:from-emerald-700 hover:to-blue-700 transition-all duration-200 ${isGenerating ? 'opacity-75 cursor-wait' : ''}`}
        title="Save as PDF"
      >
        {isGenerating ? (
          <>
