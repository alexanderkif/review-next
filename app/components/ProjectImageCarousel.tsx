...
export default function ProjectImageCarousel({ images, title }: ProjectImageCarouselProps) {
  ...
  const renderImageContent = (fullscreen: boolean = false) => {
    ...
    try {
      ...
      return (
        <Image
          ...
          fill
          className="object-contain transition-transform duration-300 hover:translate-y-[-1px]"
    ...
    <>
      ...
      <div className="relative group">
        ...
        >
          ...
          <button
            className="absolute top-4 left-4 opacity-0 group-hover:opacity-100 transition-all bg-black/50 hover:bg-black/70 text-white p-2 rounded-full z-10"
            onClick={(e) => {
              e.stopPropagation();
              toggleFullscreen();
            }}
          ...
          {images.length > 1 && (
            <>
              <button
                className="absolute left-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all bg-black/50 hover:bg-black/70 text-white p-2 rounded-full z-10"
                onClick={(e) => {
                  e.stopPropagation();
                  prevImage();
                }}
              ...
              <button
                className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all bg-black/50 hover:bg-black/70 text-white p-2 rounded-full z-10"
                onClick={(e) => {
                  e.stopPropagation();
                  nextImage();
                }}
            ...
            {images.map((image, index) => {
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 relative w-20 h-12 rounded-lg overflow-hidden transition-all ${
                    index === currentIndex
      ...
      {mounted && isFullscreen && createPortal(
        ...
        >
          {/* Кнопка закрытия */}
          <button
            className="absolute top-4 right-4 bg-black/50 hover:bg-black/70 text-white p-3 rounded-full transition-colors z-[10000]"
            onClick={toggleFullscreen}
            title="Close"
          ...
          >
            ...
            {images.length > 1 && (
              <>
                <button
                  className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-3 rounded-full transition-colors z-[10000]"
                  onClick={(e) => {
                    e.stopPropagation();
                    prevImage();
                  }}
                ...
                <button
                  className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-3 rounded-full transition-colors z-[10000]"
                  onClick={(e) => {
                    e.stopPropagation();
                    nextImage();
                  }}
