    ...
    callbacks: {
        ...
        async signIn({ user, account, profile }: { user: any; account: any; profile?: any }) {
            ...
            if (account?.provider === 'google' || account?.provider === 'github') {
                try {                    const existingUser = await getUser(user.email!);
                    ...
                    if (!existingUser) {
                        ...
                        await sql`
                            UPDATE users 
                            SET email_verified = true, avatar_url = ${user.image || existingUser.avatar_url}
            ...
            if (user) {
                ...
                if (account?.provider === 'google' || account?.provider === 'github') {
                    ...
                    if (dbUser) {
                        ...
                        token.avatar_url = dbUser.avatar_url || user.image;
                    }
                } else {
                    // For credentials
                    token.id = user.id;
