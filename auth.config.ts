...
export const authConfig = {
    ...
    callbacks: {
        authorized({ auth, request: { nextUrl } }: { auth: { user?: { role?: string } } | null; request: { nextUrl: URL } }) {
            ...
            if (account?.provider === 'google' || account?.provider === 'github') {
                try {
                    ...
                    if (!existingUser) {
                        ...
                        const avatarUrl = 'image' in user ? (user.image || existingUser.avatar_url) : existingUser.avatar_url;
            ...
            if (user) {
                ...
                if (account?.provider === 'google' || account?.provider === 'github') {
                    ...
                    if (dbUser) {
                        ...
                        token.avatar_url = dbUser.avatar_url ?? ('image' in user ? (user.image || undefined) : undefined);
                    }
                } else {
                    // For credentials
